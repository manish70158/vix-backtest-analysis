#!/usr/bin/env python3
"""
NIFTY Expiry Day VIX vs Movement Backtester V6 (Weekly + Monthly)

V6 CHANGES (based on V5):
- Adds 'move_direction' column for ALL rows
- Shows if intraday movement was "Down to Up" (bullish) or "Top to Down" (bearish)
- Direction determined by: Close > Open = "Down to Up", Close < Open = "Top to Down"
- Helps identify overall expiry day behavior AND VIX miscalculation patterns
- Statistics shown for both ALL days and significant moves (|diff| > 0.5%)

V5 CHANGES (based on V4):
- Modified threshold to 0.5% (most conservative - stricter than V4's 0.4%)
- Only marks as "Underestimated" if actual > predicted by more than 0.5%
- Otherwise marks as "Overestimated" (includes accurate predictions within 0.5%)
- Identifies only EXTREME VIX failures (>0.5% miss = 125+ points at NIFTY 25000)
...
class NiftyAllExpiriesBacktester:
    ...
    @staticmethod
    def determine_move_direction(open_price: float, high: float, low: float, close: float) -> str:
        """
        Determine the primary intraday move direction.
        ...
        Returns:
            "Down to Up" for bullish days, "Top to Down" for bearish days, "Flat" for no change
        """
        if close > open_price:
            return "Down to Up"  # Bullish day - ended higher
        elif close < open_price:
            return "Top to Down"  # Bearish day - ended lower
        else:
            return "Flat"  # No net change
    ...
    def analyze_expiry_day(self, nifty_df: pd.DataFrame, vix_df: pd.DataFrame,
        ...
        # V5 Logic: Only mark as Underestimated if difference > 0.5%
        # This creates a threshold for EXTREME underestimation (most conservative - stricter than V4)
        # 0.5% = ~125 points at NIFTY 25000 levels
        diff = actual_range_pct - vix_predicted_move_pct
        vix_accuracy = 'Underestimated' if diff > 0.5 else 'Overestimated'

        # V6 Logic: Determine move direction for ALL rows
        move_direction = self.determine_move_direction(
            nifty_data['Open'],
            nifty_data['High'],
            nifty_data['Low'],
            nifty_data['Close']
    ...
    def run_backtest(self) -> Dict:
        ...
        print(f"\n{'='*80}")
        print("NIFTY Expiry Day VIX vs Movement Backtester V6 (Weekly + Monthly)")
        ...
        for expiry_date, expiry_type in expiries:
            ...
            if analysis:
                ...
                direction_str = f" [{analysis['move_direction']}]"
                print(f"{indicator} {expiry_type.capitalize():7} {analysis['date']} ({analysis['day_of_week'][:3]}): "
                      ...
                      f"Actual {analysis['actual_range_pct']:.2f}% ({analysis['vix_accuracy']}){direction_str}")
        ...
        # V6: Add direction statistics
        stats['direction_analysis'] = self._calc_direction_stats(df_results)
    ...
    def _calc_direction_stats(self, df: pd.DataFrame) -> Dict:
        """Calculate statistics for move direction for all rows."""
        if df.empty:
            return {
                'total_moves': 0,
                'down_to_up_count': 0,
        ...
        down_to_up = len(df[df['move_direction'] == 'Down to Up'])
        top_to_down = len(df[df['move_direction'] == 'Top to Down'])
        flat = len(df[df['move_direction'] == 'Flat'])

        # Also calculate stats for significant differences (|diff| > 0.5%)
        df_significant = df[df['diff_pct'].abs() > 0.5]
        significant_down_to_up = len(df_significant[df_significant['move_direction'] == 'Down to Up'])
        significant_top_to_down = len(df_significant[df_significant['move_direction'] == 'Top to Down'])

        return {
            'total_moves': len(df),
            'down_to_up_count': down_to_up,
            'top_to_down_count': top_to_down,
            'flat_count': flat,
            'down_to_up_pct': round((down_to_up / len(df)) * 100, 1) if len(df) > 0 else 0,
            'top_to_down_pct': round((top_to_down / len(df)) * 100, 1) if len(df) > 0 else 0,
            'total_significant_moves': len(df_significant),
            'significant_down_to_up_count': significant_down_to_up,
            'significant_top_to_down_count': significant_top_to_down,
            'significant_down_to_up_pct': round((significant_down_to_up / len(df_significant)) * 100, 1) if len(df_significant) > 0 else 0,
            'significant_top_to_down_pct': round((significant_top_to_down / len(df_significant)) * 100, 1) if len(df_significant) > 0 else 0
        }
    ...
    def generate_report(self, backtest_results: Dict) -> str:
        ...
        report = []
        report.append("\n" + "="*80)
        report.append("BACKTEST RESULTS: VIX vs NIFTY Expiry Day Movement (V6)")
        ...
        # V6: Direction analysis
        if 'direction_analysis' in stats:
            dir_stats = stats['direction_analysis']
            report.append("\n🎯 DIRECTION ANALYSIS (All Expiry Days)")
            report.append("-" * 80)
            report.append(f"Total Days Analyzed: {dir_stats['total_moves']}")
            report.append(f"Down to Up (Bullish): {dir_stats['down_to_up_count']} ({dir_stats['down_to_up_pct']}%)")
            report.append(f"Top to Down (Bearish): {dir_stats['top_to_down_count']} ({dir_stats['top_to_down_pct']}%)")
            if dir_stats['flat_count'] > 0:
                report.append(f"Flat Days: {dir_stats['flat_count']}")

            if dir_stats['down_to_up_count'] > dir_stats['top_to_down_count']:
                report.append("→ Overall, expiry days are MORE BULLISH")
            elif dir_stats['top_to_down_count'] > dir_stats['down_to_up_count']:
                report.append("→ Overall, expiry days are MORE BEARISH")
            else:
                report.append("→ Overall, expiry days are BALANCED")

            # Significant moves analysis
            if dir_stats['total_significant_moves'] > 0:
                report.append("\n🔥 SIGNIFICANT MOVES ONLY (When |VIX Diff| > 0.5%)")
                report.append("-" * 80)
                report.append(f"Total Significant Moves: {dir_stats['total_significant_moves']} ({round(dir_stats['total_significant_moves']/dir_stats['total_moves']*100, 1)}% of all days)")
                report.append(f"Down to Up (Bullish): {dir_stats['significant_down_to_up_count']} ({dir_stats['significant_down_to_up_pct']}%)")
                report.append(f"Top to Down (Bearish): {dir_stats['significant_top_to_down_count']} ({dir_stats['significant_top_to_down_pct']}%)")

                if dir_stats['significant_down_to_up_count'] > dir_stats['significant_top_to_down_count']:
                    report.append("→ VIX miscalculations occur MORE on BULLISH days")
                elif dir_stats['significant_top_to_down_count'] > dir_stats['significant_down_to_up_count']:
                    report.append("→ VIX miscalculations occur MORE on BEARISH days")
                else:
                    report.append("→ VIX miscalculations occur EQUALLY on bullish and bearish days")
    ...
    def export_results(self, backtest_results: Dict, filename: str = "vix_all_expiries_results_v6.json"):
    ...
    def export_csv(self, backtest_results: Dict, filename: str = "vix_all_expiries_results_v6.csv"):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Backtest VIX predictions vs NIFTY movement on weekly + monthly expiries (V6 - with move direction)',
