#!/usr/bin/env python3
"""
NIFTY Expiry Day VIX vs Movement Backtester V3 (Weekly + Monthly)

V3 CHANGES (includes all V2 changes):
...
- Only marks as "Underestimated" if actual > predicted by more than 0.2%
- Otherwise marks as "Overestimated" (includes accurate predictions within 0.2%)
...
class NiftyAllExpiriesBacktester:
    ...
    def analyze_expiry_day(self, nifty_df: pd.DataFrame, vix_df: pd.DataFrame,
        ...
        # V2 Logic: Only mark as Underestimated if difference > 0.2%
        # This creates a threshold for "significant" underestimation
        diff = actual_range_pct - vix_predicted_move_pct
        vix_accuracy = 'Underestimated' if diff > 0.2 else 'Overestimated'
    ...
    def export_results(self, backtest_results: Dict, filename: str = "vix_all_expiries_results_v3.json"):
    ...
    def export_csv(self, backtest_results: Dict, filename: str = "vix_all_expiries_results_v3.csv"):
...
def main():
    ...
    parser.add_argument('--json', type=str, default='vix_all_expiries_results_v3.json', help='JSON output filename')
    parser.add_argument('--csv', type=str, default='vix_all_expiries_results_v3.csv', help='CSV output filename')
